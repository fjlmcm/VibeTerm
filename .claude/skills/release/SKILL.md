...
(cd src-tauri && cargo test -p vibeterm-agent-watch) # 快照解析 + 价格/窗口断言守门
```

- 刷新的是 `src-tauri/crates/vibeterm-agent-watch/src/claude/litellm_snapshot.json`(模型价格 + 上下文窗口,编译进二进制的离线兜底)。
