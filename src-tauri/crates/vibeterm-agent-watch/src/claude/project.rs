...
/// 模型 → 默认上下文窗口. 按版本号规则判定(`models::context_window_of`:4.6+ / fable /
/// mythos 为 1M,其余 200k),新型号自动落入正确一侧,取代旧版手维护前缀表和后来的
/// LiteLLM 快照 —— 前者每出新模型漏一次(opus-4-8、fable-5 都虚高过 5 倍 ctx%),
/// 后者给的是 API 上限而非默认值(sonnet-4-5 被标 1M,反向低 5 倍).
/// 优先级:
